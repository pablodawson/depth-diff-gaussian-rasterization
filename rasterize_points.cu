...
std::tuple<torch::Tensor, torch::Tensor, int, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor>
CountGaussiansCUDA(
	const torch::Tensor& background,
	const torch::Tensor& means3D,
    const torch::Tensor& colors,
	...
	const bool f_count)
{
  ...
  return std::make_tuple(gaussians_count, important_score, rendered, out_color, out_depth, radii, geomBuffer, binningBuffer, imgBuffer);
}
